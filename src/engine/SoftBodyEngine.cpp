...
void SoftBodyEngine::init(VkDevice device, VmaAllocator allocator,
                           VkDescriptorPool descriptorPool,
                           VkCommandPool cmdPool, VkQueue queue,
                           const std::string& shaderDir,
                           float worldSize, uint32_t gridRes) {
    device_    = device;
    ...
    std::cout << "[SoftBody] " << totalCount_ << " particles, "
              << totalEdgeCount_ << " edges (" << nEdgeColors_ << " colors), "
              << totalTetCount_  << " tets ("  << nTetColors_  << " colors)"
              << std::endl;
}
        ...
        for (int iter = 0; iter < solverIterations; ++iter) {
            ...
            for (int c = 0; c < nEdgeColors_; ++c) {
                ...
                vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                    kSolveEdge_.pipelineLayout, 0, 1, &ds, 0, nullptr);
                vkCmdPushConstants(cmd, kSolveEdge_.pipelineLayout,
            ...
            for (int c = 0; c < nTetColors_; ++c) {
                ...
                vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                    kSolveVolume_.pipelineLayout, 0, 1, &ds, 0, nullptr);
                vkCmdPushConstants(cmd, kSolveVolume_.pipelineLayout,
                    VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(SimPC), &pc);
                vkCmdDispatch(cmd, (cnt + 255u) / 256u, 1, 1);
            }
            computeBarrier(cmd);
...
void SoftBodyEngine::loadSBFile(InstanceData& inst) {
    ...
    if (!f.is_open())
        throw std::runtime_error("Cannot open .sb file: " + inst.sbPath);
    ...
    if (magic != SB_MAGIC)
        throw std::runtime_error("Invalid .sb file (bad magic): " + inst.sbPath);
...
void SoftBodyEngine::initGPUBuffers(VkCommandPool cmdPool, VkQueue queue) {
    ...
    auto up = [&](const std::string& name, const void* data, size_t bytes) {
        attrBuf_.upload(name, data, bytes, cmdPool, queue);
    };
    ...
    // CPU バッファは不要になったので解放
    combinedPos_.clear();        combinedPos_.shrink_to_fit();
    combinedInvMass_.clear();    combinedInvMass_.shrink_to_fit();
    combinedTypeFlag_.clear();   combinedTypeFlag_.shrink_to_fit();
    combinedEdgeData_.clear();   combinedEdgeData_.shrink_to_fit();
    combinedTetData_.clear();    combinedTetData_.shrink_to_fit();
    combinedTetRestVol_.clear(); combinedTetRestVol_.shrink_to_fit();
}
