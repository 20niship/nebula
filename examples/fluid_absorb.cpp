...
static constexpr float kMoveDur   = 10.0f;
...
static uint32_t appendCylinder(float cx, float cy, float r,
                                float zBot, float zTop, float spacing,
...
// 矩形枠の境界粒子を out に追記する
static void appendRectBorder(float x1, float y1, float x2, float y2,
                              float zBot, float zTop, float spacing,
                              std::vector<glm::vec4>& out) {
...
static void makeCylinderKinematic(float cx, float cy, float r,
                                   float zBot, float zTop, float spacing,
                                   float vx,
...
struct AbsorbArgs : public argparse::Args {
    ...
    std::string& screenshot_dir =
        kwarg("screenshot-dir", "screenshot output directory").set_default(std::string(""));
};
...
private:
    ...
    void recordComputeCmd(VkCommandBuffer cmd) {
        ...
        makeCylinderKinematic(cylX, kCylY, kCylRadius, zBot, 2.0f, spacing,
                               vx, movingPos_, movingVel_);
        engine_.recordKinematicBoundaryUpdate(cmd, base_.currentFrame,
                                               movingBoundaryStart_, nMoving_,
                                               movingPos_.data(), movingVel_.data());
    ...
    void drawFrame(int nShots) {
        ...
        float cylX = std::min(kMoveStart + kMoveSpeed * simTime_, kMoveEnd);
        engine_.setAbsorbers({FluidEngine::AbsorberDesc::CylinderZ(
            cylX, kCylY, 0.0f, kAbsRadius, 2.0f, absorbRate_)});
        ...
        ImGui::Text("FPS: %.1f  |  流体: %u / %u  経過: %.2f s",
                    ImGui::GetIO().Framerate, engine_.nFluid(),
                    engine_.config().fluidCount(), simTime_);
        ImGui::Text("移動円柱 X: %.2f m  (%.0f%%)", cylX,
                    (cylX - kMoveStart) / (kMoveEnd - kMoveStart) * 100.0f);
        ImGui::Separator();
