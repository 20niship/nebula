...
extern "C" VkResult
tryVkCreateInstance(const VkInstanceCreateInfo*,
...
void HeadlessCtx::init() {
    ...
    for (uint32_t i = 0; i < qfCount; ++i) {
        if (qf[i].queueFlags & VK_QUEUE_COMPUTE_BIT) { computeFamily = i; break; }
    }
    if (computeFamily == UINT32_MAX)
        throw std::runtime_error("HeadlessCtx: no compute queue");
    fprintf(stderr, "[HCtx] step 3 OK: computeFamily=%u\n", computeFamily); fflush(stderr);
    ...
    // ── Memory diagnostics ────────────────────────────────────────────────
    {
        ...
        for (uint32_t i = 0; i < mp.memoryHeapCount; i++)
            fprintf(stderr, "[HCtx]   heap[%u] size=%llu flags=0x%x\n",
                    i, (unsigned long long)mp.memoryHeaps[i].size, mp.memoryHeaps[i].flags);
        for (uint32_t i = 0; i < mp.memoryTypeCount; i++)
            fprintf(stderr, "[HCtx]   type[%u] heap=%u props=0x%x\n",
    ...
    if (vkCreateDevice(physicalDevice, &devCI, nullptr, &device) != VK_SUCCESS)
        throw std::runtime_error("HeadlessCtx: vkCreateDevice failed");
    ...
    if (vmaCreateAllocator(&vmaCI, &allocator) != VK_SUCCESS)
        throw std::runtime_error("HeadlessCtx: vmaCreateAllocator failed");
    ...
    if (vkCreateCommandPool(device, &poolCI, nullptr, &commandPool) != VK_SUCCESS)
        throw std::runtime_error("HeadlessCtx: vkCreateCommandPool failed");
    ...
    if (vkCreateDescriptorPool(device, &dpCI, nullptr, &descriptorPool) != VK_SUCCESS)
        throw std::runtime_error("HeadlessCtx: vkCreateDescriptorPool failed");
...
void HeadlessCtx::readBuffer(VkBuffer src, VkDeviceSize srcOffset,
                              void* dst, VkDeviceSize size) const {
