...
// ── ユーティリティ ─────────────────────────────────────────────────────────
static uint32_t floatBits(float f) { uint32_t u; std::memcpy(&u, &f, 4); return u; }
...
static VkShaderModule loadSPV(VkDevice dev, const std::string& path) {
    ...
    file.seekg(0); file.read(code.data(), sz);
...
public:
    ...
    void init(VkDevice device, VkRenderPass rp, VkDescriptorSetLayout dsLayout,
...
private:
    ...
    VkPipeline buildPipeline(VkRenderPass rp, VkShaderModule vert, VkShaderModule frag,
                              VkPrimitiveTopology topo);
};

void LineRenderer::init(VkDevice device, VkRenderPass rp, VkDescriptorSetLayout dsLayout,
                        const std::string& shaderDir) {
    device_ = device;
    ...
    if (vkCreatePipelineLayout(device_, &li, nullptr, &pipelineLayout) != VK_SUCCESS)
...
VkPipeline LineRenderer::buildPipeline(VkRenderPass rp, VkShaderModule vert, VkShaderModule frag,
                                        VkPrimitiveTopology topo) {
    ...
    blend.attachmentCount = 1; blend.pAttachments = &blendA;
    ...
    if (vkCreateGraphicsPipelines(device_, VK_NULL_HANDLE, 1, &pi, nullptr, &pipeline) != VK_SUCCESS)
        throw std::runtime_error("LineRenderer: failed to create graphics pipeline");
    return pipeline;
}
...
public:
    ...
    void init(VkDevice device, VmaAllocator allocator, VkDescriptorPool descPool,
              VkCommandPool cmdPool, VkQueue queue, const std::string& shaderDir);
...
void String2DSim::init(VkDevice device, VmaAllocator allocator, VkDescriptorPool descPool,
                       VkCommandPool cmdPool, VkQueue queue, const std::string& shaderDir) {
    device_    = device;
    ...
    attrBuf_.upload("stretchEdges", edgeData.data(),
                    sizeof(uint32_t) * edgeData.size(), cmdPool, queue);
        ...
        for (int iter = 0; iter < solverIterations; ++iter) {
            for (int c = 0; c < nColors_; ++c) {
                ...
                vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                                        kSolveStretch_.pipelineLayout, 0, 1, &ds, 0, nullptr);
                vkCmdPushConstants(cmd, kSolveStretch_.pipelineLayout,
                                   VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(SimPC), &pc);
...
void String2DSim::cleanup() {
    kPredict_.cleanup();      kSdfCollision_.cleanup();
    kHashCount_.cleanup();    kHashScanLocal_.cleanup();
    kHashScanGlobal_.cleanup(); kHashSort_.cleanup();
    kSolveDensity_.cleanup(); kSolveStretch_.cleanup();
...
void String2DApp::initVulkan() {
    ...
    sim_.init(ctx_.device, ctx_.allocator, descriptorPool_,
              ctx_.graphicsCommandPool, ctx_.graphicsQueue, SHADER_DIR_STR);

    renderer_.init(ctx_.device, ctx_.renderPass,
                   sim_.descriptorSetLayout, SHADER_DIR_STR);
...
void String2DApp::drawFrame(float dt) {
    ...
    VkResult result = vkAcquireNextImageKHR(ctx_.device, ctx_.swapchain, UINT64_MAX,
                                             f.imageAvailable, VK_NULL_HANDLE, &imageIdx);
    if (result == VK_ERROR_OUT_OF_DATE_KHR) { ctx_.recreateSwapchain(); return; }
