...
// GPU互換コライダープリミティブ(96 bytes = 24 uint32_t、buffers[colliderIdx].data[i*24 + field]): [0]type [1-3]pos [4-6]nrm/halfExt/axis/localMin [7]radius/cellSize [8-10]線速度 [11]restitution [12]friction [13-15]pad(MESH_SDF: bufIdx/res/予約) [16-19]姿勢クォータニオン [20-22]角速度 [23]pad(16-23はMESH_SDF専用、他typeは全0=無回転)
struct alignas(16) ColliderPrimitive {
  uint32_t type;           // [0]  ColliderType
  float px, py, pz;        // [1-3] 位置 / 平面上の点 / カプセル始点 / メッシュ原点(世界座標)
  float nx, ny, nz;        // [4-6] 法線(平面) / カプセル軸ベクトル / 半辺長(ボックス) / ローカルAABB最小(MESH_SDF)
  float radius;            // [7]  球/カプセル半径 / ローカルセルサイズ(MESH_SDF)
  ...
  float qx, qy, qz, qw;    // [16-19] 姿勢クォータニオン(MESH_SDF専用)。他typeは(0,0,0,1)=無回転
  float wx, wy, wz;        // [20-22] 角速度[rad/s](MESH_SDF専用)。他typeは(0,0,0)
...
inline void ColliderSet::addBox(glm::vec3 center, glm::vec3 halfExt, float restitution, float friction, glm::vec3 vel) {
  ColliderPrimitive p{};
