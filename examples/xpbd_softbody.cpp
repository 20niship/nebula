...
public:
    void run(const SoftBodyArgs& args) {
        ...
        if (!args.shot_dir.empty())
            base_.screenshotDir = args.shot_dir;
...
private:
    ...
    void initVulkan(const SoftBodyArgs& args) {
        ...
        // Bunny が存在すれば追加 (中央上部)
        {
            ...
            if (check.good()) {
                engine_.addInstance({bunnyPath, {mid, mid, high + 1.0f}, 1.0f});
            } else {
        ...
        if (engine_.totalParticleCount() == 0) {
            throw std::runtime_error(
                "No .sb files found. Generate them first:\n"
        ...
        engine_.init(base_.ctx.device, base_.ctx.allocator, base_.descriptorPool,
                     ...
                     SHADER_DIR_STR, args.world_size, uint32_t(args.grid_res));
    ...
    void recordGraphicsCmd(VkCommandBuffer cmd, uint32_t imageIdx) {
        ...
        // particle.vert は posIdx / worldMin / worldMax / particleCount を参照
        SimPC renderPc{};
        renderPc.posIdx        = engine_.posIdx;
        renderPc.velIdx        = engine_.velIdx;
        renderPc.particleCount = engine_.totalParticleCount();
        renderPc.worldMin      = 0.0f;
        renderPc.worldMax      = engine_.totalParticleCount() > 0 ? 10.0f : 10.0f;

        graphicsPipe_.draw(cmd, engine_.descriptorSet, renderPc,
                           engine_.totalParticleCount());
    ...
    void drawFrame() {
        ...
        ImGui::Text("FPS: %.1f  |  N=%u  |  t=%.2f s",
                    ...
                    engine_.totalParticleCount(), simTime_);
