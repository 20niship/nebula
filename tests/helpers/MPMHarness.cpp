...
uint32_t mpmMortonEncode(uint32_t x, uint32_t y, uint32_t z) {
    return mpmMortonExpand(x) | (mpmMortonExpand(y) << 1u) | (mpmMortonExpand(z) << 2u);
}
...
void MPMHarness::dispatchMPM(VkCommandBuffer cmd, ComputePipeline& k,
                               const MPMSimPC& pc, uint32_t count) {
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, k.pipeline);
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                            k.pipelineLayout, 0, 1,
                            &attrBuf_.descriptorSet, 0, nullptr);
...
void MPMHarness::init(const HeadlessCtx& ctx,
                      uint32_t gridRes, float worldSize,
    ...
    auto load = [&](ComputePipeline& k, const std::string& name) {
        k.init(ctx.device, attrBuf_.descriptorSetLayout, shaderDir + "/" + name + ".spv");
    };
...
void MPMHarness::uploadGrid(const std::vector<glm::vec4>& gridMom,
...
MPMSimPC MPMHarness::makePC(float dt, float rho0, float mu, float lam, float gravity) {
    ...
    defMat.q_max = 1e5f;
    attrBuf_.upload("materials", &defMat, sizeof(MaterialParams),
                    ctx_->commandPool, ctx_->computeQueue);
...
void MPMHarness::uploadMaterials(const std::vector<MaterialParams>& mats) {
    ...
    attrBuf_.upload("materials", mats.data(),
                    mats.size() * sizeof(MaterialParams),
                    ctx_->commandPool, ctx_->computeQueue);
}
...
void MPMHarness::runFullStep(const MPMSimPC& pc) {
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                            kScanLocal_.pipelineLayout, 0, 1,
                            &attrBuf_.descriptorSet, 0, nullptr);
    vkCmdPushConstants(cmd, kScanLocal_.pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT,
                       0, sizeof(MPMSimPC), &pc);
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                            kScanGlobal_.pipelineLayout, 0, 1,
                            &attrBuf_.descriptorSet, 0, nullptr);
    vkCmdPushConstants(cmd, kScanGlobal_.pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT,
                       0, sizeof(MPMSimPC), &pc);
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                            kHashAddBase_.pipelineLayout, 0, 1,
                            &attrBuf_.descriptorSet, 0, nullptr);
    vkCmdPushConstants(cmd, kHashAddBase_.pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT,
                       0, sizeof(MPMSimPC), &pc);
    vkCmdDispatch(cmd, NG, 1, 1);
    barrier(cmd);
...
float MPMHarness::readGridMass(uint32_t mortonIdx) const {
    float r = 0.0f;
    ctx_->readBuffer(attrBuf_.getBuffer("gridMass"),
                     mortonIdx * sizeof(float), &r, sizeof(r));
    return r;
}

glm::vec3 MPMHarness::readGridVel(uint32_t mortonIdx) const {
    glm::vec4 r{};
    ctx_->readBuffer(attrBuf_.getBuffer("gridMom"),
                     mortonIdx * sizeof(glm::vec4), &r, sizeof(r));
    return glm::vec3(r);
}
...
void MPMHarness::cleanup() {
    for (auto* k : {&kZeroGrid_, &kZeroCells_, &kHashCount_,
                    &kScanLocal_, &kScanGlobal_, &kHashAddBase_,
                    &kHashSort_, &kP2G_, &kGridUpdate_, &kG2P_})
        k->cleanup();
    attrBuf_.cleanup();
}
