...
void FluidEngine::init(VkDevice device, VmaAllocator allocator, VkDescriptorPool descriptorPool, VkCommandPool cmdPool, VkQueue queue, const std::string& shaderDir, const FluidConfig& cfg) {
  ...
  posIdx          = attrBuf_.addAttribute("P", sizeof(glm::vec4), totalCap);
  velIdx          = attrBuf_.addAttribute("v", sizeof(glm::vec4), totalCap);
  predPIdx        = attrBuf_.addAttribute("predP", sizeof(glm::vec4), totalCap);
  invMassIdx      = attrBuf_.addAttribute("invMass", sizeof(glm::vec4), totalCap);
...
void FluidEngine::loadBoundary(const std::string& objPath, float spacing) {
  ...
  attrBuf_.upload("P", pts.data(), sizeof(glm::vec4) * n, cmdPool_, queue_);
  attrBuf_.upload("predP", pts.data(), sizeof(glm::vec4) * n, cmdPool_, queue_);

  // invMass = 0 (固定)
  std::vector<glm::vec4> zeroMass(n, glm::vec4(0.0f));
  attrBuf_.upload("invMass", zeroMass.data(), sizeof(glm::vec4) * n, cmdPool_, queue_);

  // 速度 = 0
  std::vector<glm::vec4> zeroVel(n, glm::vec4(0.0f));
  attrBuf_.upload("v", zeroVel.data(), sizeof(glm::vec4) * n, cmdPool_, queue_);
...
void FluidEngine::loadBoundary(const std::string& objPath, float spacing, float scale, glm::vec3 offset, bool yup_to_zup) {
  ...
  // 境界パーティクルは常に buffer index 0 から書き込む（固定領域）
  attrBuf_.upload("P", mesh.particles.data(), sizeof(glm::vec4) * n, cmdPool_, queue_);
  attrBuf_.upload("predP", mesh.particles.data(), sizeof(glm::vec4) * n, cmdPool_, queue_);
...
// ── GPUパス単位プロファイリング(診断用; GpuProfilerに委譲) ─────────────────────
#ifdef NEBULA_GPU_PROFILING

void FluidEngine::enableGpuProfiling(VkPhysicalDevice physicalDevice) {
  profiler_.enable(device_, physicalDevice);
  attrBuf_.enableGpuProfiling(device_, physicalDevice); // 転送(vkCmdCopyBuffer)も計測対象にする
}

void FluidEngine::printGpuProfile() {
  profiler_.print("FluidEngine GPU compute profile");
  attrBuf_.printGpuProfile(); // AttributeBuffer側でタイトル出力済み
}
#endif
...
void FluidEngine::step(VkCommandBuffer cmd, float dt) {
  ...
  for(int sub = 0; sub < numSubsteps; ++sub) {
    ...
    profiler_.end(cmd, "ZeroCells");

    profiler_.begin(cmd);
    kHashCount_.dispatch(cmd, ds, pc, totalN);
    computeBarrier(cmd);
    profiler_.end(cmd, "HashCount");
    ...
    computeBarrier(cmd);
    profiler_.end(cmd, "HashScanLocal");
    ...
    profiler_.end(cmd, "HashScanGlobal");
    ...
    computeBarrier(cmd);
    profiler_.end(cmd, "HashAddBase");

    profiler_.begin(cmd);
    kHashSort_.dispatch(cmd, ds, pc, totalN);
    computeBarrier(cmd);
    profiler_.end(cmd, "HashSort");
    ...
    for(int iter = 0; iter < pbfIterations; ++iter) {
      ...
      computeBarrier(cmd);
      profiler_.end(cmd, "PbfDensity");
      profiler_.begin(cmd);
      kPbfDeltaP_.dispatch(cmd, ds, pc, nFluid_);
      computeBarrier(cmd);
      profiler_.end(cmd, "PbfDeltaP");
    }
    ...
    if(foamEnabled && cfg_.maxDiffuseParticles > 0) {
      ...
      profiler_.end(cmd, "FoamGenerate");
      profiler_.begin(cmd);
      kFoamAdvect_.dispatch(cmd, ds, pc, cfg_.maxDiffuseParticles);
      profiler_.end(cmd, "FoamAdvect");
    }
