...
inline glm::vec3 closestPtOnTriangle(glm::vec3 p,
...
inline uint32_t meshSdfMortonEncode(uint32_t x, uint32_t y, uint32_t z) {
  return meshSdfMortonExpand(x) | (meshSdfMortonExpand(y) << 1u) | (meshSdfMortonExpand(z) << 2u);
}
...
inline std::vector<float> buildMeshSDF(const std::vector<MeshTriangle>& tris,
...
inline std::vector<MeshTriangle> transformTriangles(const std::vector<MeshTriangle>& tris,
  ...
  for (size_t i = 0; i < tris.size(); ++i) {
    ...
    for (int j = 0; j < 3; ++j)
      out[i].v[j] = rotation * tris[i].v[j] + translation;
  }
  return out;
}
