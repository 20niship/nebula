...
public:
  ...
  void init(const HeadlessCtx& ctx, const Config& cfg, const std::string& shaderDir, const std::vector<glm::vec4>& initPos, const std::vector<glm::vec4>& initInvMass);
  ...
  static void buildChainEdges(uint32_t N, float spacing, std::vector<uint32_t>& edgeDataOut, uint32_t& color0EndOut);
