...
static float bspline2(float d) {
    ...
    if (ad < 1.5f) { float t = 1.5f - ad; return 0.5f * t * t; }
    return 0.0f;
}
...
TEST_CASE("MPM Constitutive - 5-1: Identity F => zero stress") {
    ...
    for (int row = 0; row < 3; ++row)
        CHECK(std::abs(tau[col][row]) < 1e-4f);
}
...
TEST_CASE("MPM GPU - 4-1: G2P uniform velocity field") {
    ...
    for (int ix = 0; ix < 3; ++ix) {
        ...
        if (nx < 0 || ny < 0 || nz < 0 ||
            nx >= int(sim.gridRes()) || ny >= int(sim.gridRes()) ||
            nz >= int(sim.gridRes())) continue;
...
TEST_CASE("MPM GPU Advanced - 7-2: G2P velocity gradient updates F matrix") {
    ...
    for (int ix = 0; ix < 3; ++ix) {
        ...
        if (nx < 0 || ny < 0 || nz < 0 ||
            nx >= int(sim.gridRes()) || ny >= int(sim.gridRes()) || nz >= int(sim.gridRes())) continue;
        uint32_t m  = mpmMortonEncode(uint32_t(nx), uint32_t(ny), uint32_t(nz));
...
TEST_CASE("MPM GPU Advanced - 7-3: Multi-particle momentum conservation (no gravity, tau=0)") {
    ...
    // p0: x正向き, p1: x負向き (対称)
    particles[0].pos = {4.0f, 5.0f, 5.0f}; particles[0].vel = { 2.0f,  0.0f, 0.0f};
    particles[1].pos = {6.0f, 5.0f, 5.0f}; particles[1].vel = {-2.0f,  0.0f, 0.0f};
    // p2: y正向き, p3: y負向き (対称)
    particles[2].pos = {5.0f, 4.0f, 5.0f}; particles[2].vel = { 0.0f,  2.0f, 0.0f};
    particles[3].pos = {5.0f, 6.0f, 5.0f}; particles[3].vel = { 0.0f, -2.0f, 0.0f};
