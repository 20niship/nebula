...
void PBFHarness::init(const HeadlessCtx& ctx, const Config& cfg,
                      const std::string& shaderDir,
                      const std::vector<glm::vec4>& initPos,
                      const std::vector<glm::vec4>& initInvMass,
                      const std::vector<glm::vec4>& boundaryPos) {
    ctx_ = &ctx;
    cfg_ = cfg;
    ...
    if (!boundaryPos.empty()) {
        ...
        attrBuf_.uploadAt("P",     boundaryPos.data(), sizeof(glm::vec4)*nb,
                          cfg_.N * sizeof(glm::vec4), pool, queue);
        attrBuf_.uploadAt("predP", boundaryPos.data(), sizeof(glm::vec4)*nb,
                          cfg_.N * sizeof(glm::vec4), pool, queue);
    }
...
void PBFHarness::recordSubstep(VkCommandBuffer cmd, float subDt) {
...

    {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                                kScanLoc_.pipelineLayout, 0, 1, &ds, 0, nullptr);
        vkCmdPushConstants(cmd, kScanLoc_.pipelineLayout,
...

    {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                                kScanGlob_.pipelineLayout, 0, 1, &ds, 0, nullptr);
        vkCmdPushConstants(cmd, kScanGlob_.pipelineLayout,
                           VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(SimPC), &pc);
...
void PBFHarness::cleanup() {
    ...
    kHashCnt_.cleanup();    kScanLoc_.cleanup();
    kScanGlob_.cleanup();   kSort_.cleanup();
    kPbfDensity_.cleanup(); kPbfDeltaP_.cleanup();
    kPbfViscosity_.cleanup();kUpdateVel_.cleanup();
    attrBuf_.cleanup();
}
