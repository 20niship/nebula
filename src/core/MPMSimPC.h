...
// MPM 専用 Push Constants — 180 bytes。GLSL側push_constantはvec3/uvec3を16byte境界に強制配置するため、各vec3系フィールド直前のスカラー数を4の倍数に揃えてパディングなしで境界を一致させている(フィールド順・スカラー数を変える場合は要再検証)。hashCellsは旧gridRes(スカラー)を改名したもので空間ハッシュ/MPMグリッドバッファの実要素数(=domain::hashCells()、cubeRes^3。nx*ny*nzではない)を表す。
struct MPMSimPC {
  ...
  uint32_t gridVelOldIdx; // 132 packed-half3×CELLS (8B/cell) FLIP用 v_old (旧NanoVDB SDF境界条件用reserved)
  uint32_t gridMomIdx;    // 136 vec4×CELLS P2G(scatter)固定小数点atomicAdd蓄積専用、G2Pは読まない
  uint32_t gridMassIdx;   // 140 float×CELLS 同上
  ...
  uint32_t gridVelIdx;    // 176 packed-half3×CELLS (8B/cell) GridUpdateが書きG2Pが読む v_new
};
static_assert(sizeof(MPMSimPC) == 180, "MPMSimPC must be 180 bytes");
