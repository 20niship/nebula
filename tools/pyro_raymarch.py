...
.pvox フォーマット (PyroEngine::dumpFrame と対応):
  header: magic"PVX1"(4B), nx,ny,nz(u32×3), cellSize(f32), simTime(f32),
          numChannels(u32), [name(16B ascii固定長), components(u32)]×numChannels
  body:   float32 データ (チャンネル毎、線形 x+y*nx+z*nx*ny 順)

  issue #46フォローアップ: 直方体ドメイン対応につき nx,ny,nz は個別の値を持つ。旧
  worldSize(単一float、全軸共通の物理サイズ)は cellSize(全軸共通のセルサイズ)に変更した
  (バイト数は不変)。軸の物理長は dims[axis] * cellSize で求める。
...
def load_pvox(path):
    ...
    cell_size, sim_time = struct.unpack_from("<2f", data, 16)
...
def setup_rays(vox, axis, width, height):
    ...
    return dims, grid_pos, ray_axis, cell_size
...
def render(vox, axis, width, height, steps, absorption, exposure):
    ...
    dims, grid_pos, ray_axis, cell_size = setup_rays(vox, axis, width, height)
    axis_names = ["x", "y", "z"]

    color = np.zeros((height, width, 3), dtype=np.float32)
    transmittance = np.ones((height, width), dtype=np.float32)

    # marching軸の実長さ (セル数×cellSize) をレイステップに使う (直方体ドメイン対応)
    ds_world = dims[axis_names[ray_axis]] * cell_size / steps
