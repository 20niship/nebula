...
void PhysicsHarness::buildChainEdges(uint32_t N, float spacing,
                                      std::vector<uint32_t>& ed,
                                      uint32_t& c0End) {
    ...
    for (uint32_t i = 0; i + 1 < N; i += 2) {
        ed.push_back(i); ed.push_back(i + 1); ed.push_back(fb(spacing));
    }
...
void PhysicsHarness::init(const HeadlessCtx& ctx, const Config& cfg,
    ...
    edgesIdx_   = attrBuf_.addAttribute("edges",   sizeof(uint32_t),
                                         std::max(nEdges_ * 3u, 3u));
    lambdaIdx_  = attrBuf_.addAttribute("lambdas", sizeof(float),
                                         std::max(nEdges_, 1u));
    ...
    if (nEdges_ > 0) {
        attrBuf_.upload("edges", cfg.edgeData.data(),
                         sizeof(uint32_t) * cfg.edgeData.size(), pool, queue);
    }
    ...
    auto load = [&](ComputePipeline& k, const char* name) {
        k.init(ctx.device, attrBuf_.descriptorSetLayout, shaderDir + "/" + name);
    };
        ...
        for (int iter = 0; iter < cfg_.solverIterations; ++iter) {
            auto dispatchColor = [&](uint32_t start, uint32_t end) {
                ...
                vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                                        kSolveSt_.pipelineLayout, 0, 1, &ds, 0, nullptr);
                vkCmdPushConstants(cmd, kSolveSt_.pipelineLayout,
...
void PhysicsHarness::cleanup() {
    kPredict_.cleanup();  kSdf_.cleanup();
    kHashCnt_.cleanup();  kScanLoc_.cleanup(); kScanGlob_.cleanup(); kSort_.cleanup();
    kSolveDen_.cleanup(); kSolveSt_.cleanup(); kUpdateVel_.cleanup();
    attrBuf_.cleanup();
}
